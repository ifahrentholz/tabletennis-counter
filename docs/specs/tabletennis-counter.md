...
2. Als Spieler möchte ich vor dem Match auswählen, wie viele gewonnene Spiele
   einen Satz entscheiden (Presets 3/5/6/7, Standard 6), damit ich die Länge
...
- **Domänenmodell / Hierarchie:** `Match` besteht aus konfigurierten
  ...
  3, `playerAName`, `playerBName`) sowie einer Liste von `Set`s (Sätze).
  Ein `Set` besteht aus einer Liste von `Game`s (Spielen). Ein `Game` besteht
  aus einem laufenden Punktestand je Spieler.
- **Spiel-Gewinnregel (Deuce):** Ein Spiel ist gewonnen, sobald ein Spieler die
...
- **Navigations-/Screen-Struktur:**
  ...
  4. Satz-Detail / Spiele-Übersicht (Spiele-Stand je Spieler innerhalb dieses
     Satzes, Liste der Spiele, Editieren-Button für den Spiele-Stand)
