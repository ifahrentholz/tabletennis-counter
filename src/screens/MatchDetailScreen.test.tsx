...
describe('MatchDetailScreen labelling', () => {
  it('names itself and its list after the level it shows — sets inside the match', async () => {
    const matchId = await seedMatch(createMatch(makeConfig()));
    await render(
      <MatchDetailScreen matchId={matchId} onOpenGamesOverview={jest.fn()} onBack={jest.fn()} />,
    );

    expect(await screen.findByText('Matchübersicht')).toBeOnTheScreen();
    expect(screen.getByText('Satzverlauf')).toBeOnTheScreen();
    // The list's right-hand column counts what a set is made of.
    expect(screen.getByText('Spiele')).toBeOnTheScreen();
    expect(screen.getByText('Gesamtstand')).toBeOnTheScreen();
  });

  it('states the match format in sets', async () => {
    const matchId = await seedMatch(createMatch(makeConfig({ setsToWinMatch: 3 })));
    await render(
      <MatchDetailScreen matchId={matchId} onOpenGamesOverview={jest.fn()} onBack={jest.fn()} />,
    );

    expect(await screen.findByText('Best of 5')).toBeOnTheScreen();
  });
});
