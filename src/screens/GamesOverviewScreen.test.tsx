...
describe('GamesOverviewScreen labelling', () => {
  it('names itself, its scoreboard and its list after the level it shows — games inside one set', async () => {
    ...
    expect(await screen.findByText('Spielübersicht')).toBeOnTheScreen();
    expect(screen.getByText('Spielstand')).toBeOnTheScreen();
    expect(screen.getByText('Spielverlauf')).toBeOnTheScreen();
    expect(screen.getByText('Punkte')).toBeOnTheScreen();
  });

  it('titles itself with the set it belongs to, and states that set’s format in games', async () => {
    const matchId = await seedMatch(createMatch(makeConfig({ gamesToWinSet: 6 })));
    ...
    expect(await screen.findByText('Satz 1')).toBeOnTheScreen();
    expect(screen.getByText('First to 6')).toBeOnTheScreen();
  });
});
